...
2. 必要なライブラリやパッケージを`pip`ツールでインストールする
3. `VSCode`にPython関連の拡張機能を追加する
4. `VSCode`でプログラムを編集する
5. `VSCode`画面で実行ボタン（▷）を押すと実行することができる
   また、コマンドラインで「`python main1.py`」のようにプログラムを実行することもできる
